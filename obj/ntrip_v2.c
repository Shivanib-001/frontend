   ...
   // int g_diffsat;
    ...
    while (1) {
        ...
        if (count > 0) {
            ...
            char *byte_r = (char *)malloc(count);
            if (byte_r == NULL) {
               perror("Failed to allocate memory");
             }
              memcpy(byte_r,serial_buffer,count);
            //printf("Data received: %s\\n", serial_buffer);
            // Protect shared buffer with a mutex before writing
            gnss_filter(byte_r,&gnss_t,&v_gnss,&gnss_lat,&l_dir,&gnss_lng,&lng_dir,&gga_alt,&gga_hdop,&rmc_spd,&rmc_head,&g_qa,&g_numst,&g_diffsat,&date_g);
            //usleep(5000); // 500,000 microseconds = 0.5 seconds
            free(byte_r);
            //printf("%f,%f,%d,%f %f %d %d \n",gnss_lat,gnss_lng,gnss_t,rmc_spd,rmc_head,g_qa,g_diffsat);
            if(gnss_lat != 0.0 && gnss_lng !=0.0 && gga_alt !=0.0){
            convert_time_to_UTC((unsigned)gnss_t,&hr,&mi,&se);
            sprintf(gns_time,"%02d:%02d:%02d",hr,mi,se);
            //printf("%f,%f,%d,%f %f %d %d \n",gnss_lat,gnss_lng,gnss_t,rmc_spd,rmc_head,g_qa,g_diffsat);
            write_json_to_file("test.json",n,gns_time,gnss_lat,gnss_lng,gga_alt,gga_hdop,rmc_spd,rmc_head,g_qa,g_numst,g_diffsat,date_g);
            }
